#ifndef ENGINE_PHYSICS_SHAPE_H_
#define ENGINE_PHYSICS_SHAPE_H_
...
struct ShapeInterface {
  public:
    ...
    virtual ShapeInterface * Clone(void) const = 0;
...
struct Circle : public ShapeInterface {
  public:
    inline explicit Circle(real32 const& _radius) { radius = _radius; }
    inline Circle(void) = delete;
    inline Circle(Circle && _other) = delete;
    inline Circle(Circle const& _other) = delete;
    inline Circle & operator=(Circle && _other) = delete;
    inline Circle & operator=(Circle const& _other) = delete;
    inline virtual ~Circle(void) = default;

    ShapeInterface * Clone(void) const { return new Circle(radius); }
...
struct PolygonShape : public ShapeInterface {
  public:
    inline void Initialize(void) { ComputeMass(1.0f); }

    ShapeInterface *Clone(void) const     {
    ...
    void ComputeMass(real32 const& _density)     {
      ...
      for (uint32 i = 0; i < m_vertexCount; ++i) m_vertices[i] -= c;
    ...
    void Set(Vector2 *vertices, uint32 count) {
      ...
      for (uint32 i = 1; i < count; ++i) {
        ...
        if (x > highestXCoord) {
          highestXCoord = x;
          rightMost = i;
        } else if (x == highestXCoord && vertices[i].y < vertices[rightMost].y) rightMost = i;
      }
      ...
      for (;;) {
        ...
        for (int32 i = 1; i < (int32)count; ++i) {
          ...
          if (c == 0.0f && e2.LenSqr() > e1.LenSqr()) nextHullIndex = i;
        }
      ...
      for (uint32 i = 0; i < m_vertexCount; ++i) m_vertices[i] = vertices[hull[i]];
