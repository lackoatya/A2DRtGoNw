...
struct Vector2
{
	...
	Vector2() = default;
	Vector2(real32 const& _x, real32 const& _y) : x(_x), y(_y) { }
...
struct Mat2
{
	...
	inline Mat2 Abs(void) const { return Mat2(std::abs(m00), std::abs(m01), std::abs(m10), std::abs(m11)); }
	...
	inline const Vector2 operator*(Vector2 const& _rhs) const { return Vector2(m00 * _rhs.x + m01 * _rhs.y, m10 * _rhs.x + m11 * _rhs.y); }
