...
void Engine::Physics::Reality::IntegrateForces(Engine::Physics::Body *b, real32 const& _interval) {
  if (b->im == 0.0f) return;
...
void Engine::Physics::Reality::IntegrateVelocity(Engine::Physics::Body *b, real32 const& _interval) {
  if (b->im == 0.0f) return;
...
void Engine::Physics::Reality::Process(void) {
  ...
  for (uint32 i = 0; i < bodies_.size(); ++i)   {
    ...
    for (uint32 j = i + 1; j < bodies_.size(); ++j)     {
      Body *B = bodies_[j];
      if (A->im == 0 && B->im == 0) continue;

      Manifold m(A, B);
      m.Solve();
      if (m.contact_count) contacts_.emplace_back(m);
    }
  }

  // Integrate forces
  for (uint32 i = 0; i < bodies_.size(); ++i)
    IntegrateForces(bodies_[i], interval_);

  // Initialize collision
  for (uint32 i = 0; i < contacts_.size(); ++i)
    contacts_[i].Initialize();

  // Solve collisions
  for (uint32 j = 0; j < m_iterations; ++j)
    for (uint32 i = 0; i < contacts_.size(); ++i)
      contacts_[i].ApplyImpulse();

  // Integrate velocities
  for (uint32 i = 0; i < bodies_.size(); ++i)
    IntegrateVelocity(bodies_[i], interval_);

  // Correct positions
  for (uint32 i = 0; i < contacts_.size(); ++i)
    contacts_[i].PositionalCorrection();
...
Engine::Physics::Body * Engine::Physics::Reality::Add(ShapeInterface * shape, uint32 x, uint32 y) {
  assert(shape);
