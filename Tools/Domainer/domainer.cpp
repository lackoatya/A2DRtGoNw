...
void process_file(boost::filesystem::path _file, std::string const& _domain)
{
	...
	std::string content((std::istreambuf_iterator<char>(original_file)), std::istreambuf_iterator<char>());
	original_file.close();
...
void process_path(boost::filesystem::path const& current_path, std::string const& _domain)
{
	...
	BOOST_FOREACH(boost::filesystem::path const& path, std::make_pair(iterator, eod))
	{
		...
		{
			if (path.extension().generic_string() == ".h" || path.extension().generic_string() == ".cpp" || path.extension().generic_string() == ".hpp") process_file(path, _domain);
		}
		else
		{
			if (boost::filesystem::is_directory(path)) process_path(path, _domain);
		}
	}
}
...
int main(int argc, char *argv[])
{
	if (argc == 2)
	{
		...
		std::string domain((std::istreambuf_iterator<char>(domain_file)), std::istreambuf_iterator<char>());
		domain_file.close();
